...
let result = function Error (`Msg s) -> gfx_error "%s" s | Ok x -> x
...
let fold1 f l =
  match l with [] -> None | r :: ll -> Some (List.fold_left f r ll)
...
let create s =
  ...
  let w_flags =
    match w_flags with None -> Sdl.Window.input_focus | Some f -> f
  in
...
let main_loop f =
  let res = ref true in
  while !running && !res do
    res := f (Int32.to_float (Sdl.get_ticks ()))
  done
