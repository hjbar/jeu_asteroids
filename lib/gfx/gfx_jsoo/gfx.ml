  ...
  | Some canvas ->
    ...
    canvas##.onmouseover :=
      Dom_html.handler (fun e ->
          ...
          Js._true );
    ignore
      (Dom_html.addEventListener canvas
         (Dom_events.Typ.make "contextmenu")
         (Dom_html.handler (fun e ->
              Dom.preventDefault e;
              Js._false ) )
         Js._true );
...
let load_file url_s =
  if starts_with "VAR:" url_s then load_from_var (chop_prefix "VAR:" url_s)
    ...
    let is_ready () =
      match xhr##.readyState with XmlHttpRequest.DONE -> true | _ -> false
    in
    let get () =
...
let open_formatter src =
  ...
  with Not_found -> failwith ("No element with id " ^ src)
